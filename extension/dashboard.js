...
document.addEventListener("click",async e=>{if(!e.target.classList.contains("edit-status"))return;const status=prompt("Enter status: correct, wrong, or unreviewed", "correct");if(status){await fetch(`${API}/predictions/${e.target.dataset.id}/review`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({status})});refresh();}});
